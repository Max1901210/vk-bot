...
# для каждого жанра — цепочка моделей: если одна занята/удалена, берём следующую
GENRES = {
    "auto": ("🌐 Универсальная", [
...
def ai_chat(messages: list, uid: int | None = None) -> str:
    """Задаём вопрос нейросети (не стриминг — просто ждём ответ)."""
    p = PROVIDERS[SETTINGS["provider"]]
    genre = USER_GENRE.get(uid) if uid else None
...
WELCOME = (
    ...
    "🧠 «Модель» — выберу профиль ИИ под твою задачу\n"
